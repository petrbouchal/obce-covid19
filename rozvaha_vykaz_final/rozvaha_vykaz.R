...
mun_database <- read_excel("mun_database2.xlsx")
kraje_okresy<-read_excel("kraje_okresy.xlsx")
...
rm(kraje_okresy)
mun_data <- read_excel("mun_database.xlsx")
...
rozvaha_codelist <- read_excel("rozvaha_codelist_10-11.xlsx")
...
rozvaha_codelist <- read_excel("rozvaha_codelist.xlsx")
rozvaha_codelist<-as.data.frame(rozvaha_codelist)
colnames(balancesheet)[11]<-"polozka"
balancesheet$polozka<-rozvaha_codelist[match(balancesheet$polvyk,rozvaha_codelist$code),2] #assign names
rm(rozvaha_codelist)
...
inhab_categ<-read_excel("inhabitants_category.xlsx")
...
# ukazatele (nový dataset na ukazatele: Podíl cizích zdroju a Celková likvidita)
    # první polozku beru jako subset balance_sheet a dál pres match pridávám dalsí promenné
    # ukazatele <- hodnoty ukazatelu a promenné potrebné k jejich výpoctu (pro vsechny roky a vsechny obce)
...
ukazatele$kratkodob_zavazky<- balance_sheet[match(paste(ukazatele$mun_id,ukazatele$per_yr,"Krátkodobé závazky"),
                                              paste(balance_sheet$mun_id, balance_sheet$per_yr,
                                                    balance_sheet$polozka)),9]
ukazatele$celkova_likvidita<-ukazatele$obezna_aktiva/ukazatele$kratkodob_zavazky

## VÝKAZ ZISKU A ZTRÁT
    # loop na stazení vsech datasetu, vyhodí dataset pro kazdý rok: "vyk_2010" atd.
    # dále je dataset zpracován podobne jako Rozvaha (akorát pouzívám ofic. císelník na jména polozek)
...
inhab_categ<-read_excel("inhabitants_category.xlsx") # dataset with numbers of inhabitants
inhab_categ<-as.data.frame(inhab_categ) 
names(inhab_categ)<-c("per_yr","mun_id","categ","inhabitants","katobyv_id")
...
# vec navíc: grafy ukazatelu co jsem posílal mailem
  # Podíl cizích zdroju
