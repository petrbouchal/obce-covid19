...
iscn_bil_diffs <- iscn %>% 
  ...
  mutate(bilance_rel_diff = bilance_rel - bilance_rel_base,
         bilance_diff = bilance - bilance_base,
...
bil_impact_bar <- function(data, grpvar) {
  data %>% 
    ungroup() %>% 
    mutate(grp = {{grpvar}}) %>% 
...
bil_impact_bar(iscn_bil_diffs, typobce_wrapped) +
...
bth <- shady_bil_density(iscn, scto, typobce_wrapped) +
  labs(title = "Rozložení bilance obcí, 2019 a scénář 2020", 
       subtitle = str_glue("Scénář 2020 = {rud_drop_perc} pokles RUD."),
       caption = "Obce s bilancí nad +100 % vynechány")

plot_save(bth, "typ-hist")
plot_load("typ-hist")
```
...
bil_impact_bar(iscn_bil_diffs, kraj_short) +
  labs(title = "Podíl obcí, které se propadnou do deficitu",
