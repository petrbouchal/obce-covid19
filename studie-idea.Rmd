...
-   Deficitní rozpočet mělo `r pc(1 - bil_pos_share)` obcí (průměrný deficit u nich byl `r pc(bil_neg_avg)`, deficit v součtu `r nm(bil_neg_sum)` mld. korun), ostatní obce měly vyrovnaný nebo přebytkový rozpočet (průměrný přebytek u nich `r pc(bil_pos_avg, .1)` a v součtu `r nm(bil_pos_sum)` mld. korun).
...
```{r dluh-vs-deficit, fig.cap = "Tříletý deficit vůči počáteční výši dluhu"}
indik_last3 %>%
  group_by(ico) %>% 
...
Grafy níže rozdělují obce do čtyř kategorií podle příslušnosti do aglomerací krajských měst, případně příslušnosti k sociálně a hospodářsky vyloučeným oblastem, jak je definuje Strategie regionálního rozvoje ČR (SRR21+).
