...
indik_last3 %>% group_by(ico) %>% 
  ...
  labs(title = "Meziroční rozpočtové výkyvy", 
       caption = "Osy zobrazují relativní bilanci k příjmům v letech t a t-1")
```
...
ggplot(indik_slctyr) +
  ...
  theme_studie("y") +
  scale_y_percent_cz() +
  labs(title = "Deficit (relativní k velikosti příjmů) a velikost populace", 
       caption = "Vodorovná osa představuje logaritmus počet obyvatel")
```
...
ggplot(indik_slctyr_res) +
  ...
  colorblindr::scale_colour_OkabeIto(order = c(7, 5, 2, 8),
                                     name = NULL, darken = 0) +
  theme_studie("y") +
  labs(title = "Rezervy podle populace", 
       caption = 'Vodorovná osa představuje logaritmus počtu obyvatel,\n svislá rezervy vůči výdajům')
```
...
ggplot(indik_slctyr) +
  geom_point(aes(log(as.integer(pocob)), dluh / rozp_p_celkem, 
                 colour = typobce_wrapped)) +
  colorblindr::scale_colour_OkabeIto(order = c(7, 5, 2, 8),
                                     name = NULL, darken = 0) +
  theme_studie("y") +
