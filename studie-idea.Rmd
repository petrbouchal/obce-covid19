...
ggplot(indik_slctyr, aes(x=log(as.numeric(pocob)), colour=typobce_wrapped,
  ...
  colorblindr::scale_colour_OkabeIto(order = c(7, 5, 2, 8),
                                    name = NULL, darken = 0) +
  labs(title = "Populační rozdělení obcí podle jednotlivých kategorií", caption = 'Vodorovná osa představuje logaritmus počet obyvatel')
```
...
ggplot(indik_slctyr) +
  ...
  colorblindr::scale_colour_OkabeIto(guide = "none", order = c(7, 5, 2, 8),
                                    name = NULL, darken = 0) +
  theme_studie("x") +
  labs(title = "Kapitálové výdaje na obyvatele (Kč)", caption = "Vodorovná osa představuje logaritmus kapitálových výdajů na obyvatele")
```
...
indik_last3 %>% group_by(ico) %>% mutate(bilance_p_lag = lag(bilance/rozp_p_celkem, n=1), bilance_p_rel = bilance/rozp_p_celkem) %>% select(bilance, bilance_p_rel, bilance_p_lag, per_yr, ico, typobce_wrapped) %>%
ggplot() + geom_point(aes(bilance_p_lag, bilance_p_rel, colour = typobce_wrapped)) +
colorblindr::scale_colour_OkabeIto(order = c(7, 5, 2, 8),
                                    name = NULL, darken = 0) +
theme_studie("y") +
scale_x_percent_cz() +
scale_y_percent_cz() + labs(title = "Meziroční rozpočtové výkyvy", caption = "Osy zobrazují relativní bilanci k příjmům v letech t a t-1")
```
...
indik_last3 %>%
    group_by(ico) %>% summarise(bilance_sum = sum(bilance), prijmy_sum = sum(rozp_p_celkem)) %>%     merge(., indik_last3, by='ico') %>% filter(per_yr==2017)  %>% select(ico, obec, bilance_sum, prijmy_sum, dluh, typobce_wrapped, rozp_p_celkem) %>%
  ggplot() +
  geom_point(aes(dluh / rozp_p_celkem, bilance_sum / prijmy_sum, colour = typobce_wrapped)) + colorblindr::scale_colour_OkabeIto(order = c(7, 5, 2, 8),
                                    name = NULL, darken = 0) +
  theme_studie("y") +
  scale_x_percent_cz() +
  scale_y_percent_cz() +
  labs(title = "3-letý definicit vůči počáteční výši dluhu", caption = "Vodorovná osa zobrazuje relativní zadlužení obce vůči příjmu v roce 2017,\n svislá osa bilanci rozpočtu v letech 2017-2019 relativně k příjmům za stejné období")
```
...
#            prumer = mean(rezerva_to_spend, na.rm = T))
  ...
  labs(title = "Rezervy obcí podle typu",
       subtitle = "Jako % ročních výdajů, rok 2019", caption = "Obce s pozitivní bilancí nad 300 % výdajů vynechány")
```
...
ggplot(indik_slctyr_res) +
  geom_boxplot(aes(y= katobyv_nazev, x = rezerva_to_spend, colour=katobyv_nazev, fill = after_scale(alpha(colour, 0.7))) , outlier.alpha = .2)  +
  scale_x_percent_cz() +
  theme_studie() +
  theme(legend.title=element_blank()) +
...
labs(title = "Rezervy podle počtu obyvatel", y = "Počet obyvatel - kategorie",
...
ggplot(indik_geo) +
  geom_boxplot(aes(y = typobce_aglomerace, x = rozp_v_kap/rozp_v_celkem, colour = typobce_aglomerace,
               fill = after_scale(alpha(colour, 0.7))), outlier.alpha = .2) +
  colorblindr::scale_colour_OkabeIto(guide = "none", order = c(7, 5, 2, 8),
                                    name = NULL, darken = 0) +
  scale_x_percent_cz() +
...
Následující grafy srovnávájí vybrané ukazatele ve třech největších metropolitních oblastech (Praha, Brno, Ostrava) a ostatních aglomeracích. Grafy \@ref(fig:kap-ratio-aglo) a \@ref(fig:dluh-podil-aglo) neukazují na výrazné rozdíly mezi obcemi v metropolitních oblastech a běžných aglomeracích.

```{r kap-ratio-aglo, fig.asp = 0.4, fig.cap="Kapitálové výdaje v aglomeracích a metropolitních oblastech"}
ggplot(indik_aglo) +
  geom_boxplot(aes(y = typ_aglomerace, x = rozp_v_kap/rozp_v_celkem, colour = typ_aglomerace,
               fill = after_scale(alpha(colour, 0.7))), outlier.alpha = .2) +
  colorblindr::scale_colour_OkabeIto(guide = "none", order = c(7, 5, 2, 8),
                                    name = NULL, darken = 0) +
  scale_x_percent_cz() +
...
```{r deficit-populace, fig.cap="Deficit a velikost populace", fig.asp=0.4}
 ggplot(indik_slctyr) +
  ...
  geom_point(aes(log(as.integer(pocob)), bilance/rozp_p_celkem, colour =typobce_wrapped)) +
  colorblindr::scale_colour_OkabeIto(order = c(7, 5, 2, 8),
                                    name = NULL, darken = 0) +
  theme_studie("y") +
  scale_y_percent_cz() +
  labs(title = "Deficit (relativní k velikosti příjmů) a velikost populace", caption = "Vodorovná osa představuje logaritmus počet obyvatel")
```
...
ggplot(indik_slctyr_res) +
  geom_point(aes(log(as.integer(pocob)), rezerva_to_spend, colour = typobce_wrapped)) +
  colorblindr::scale_colour_OkabeIto(order = c(7, 5, 2, 8),
                                    name = NULL, darken = 0) +
  theme_studie("y") +
  labs(title = "Rezervy podle populace", caption = 'Vodorovná osa představuje logaritmus počtu obyvatel,\n svislá rezervy vůči výdajům')
```
...
ggplot(indik_slctyr) +
  geom_point(aes(log(as.integer(pocob)), dluh / rozp_p_celkem, colour = typobce_wrapped)) +
  colorblindr::scale_colour_OkabeIto(order = c(7, 5, 2, 8),
                                    name = NULL, darken = 0) +
  theme_studie("y") +
  labs(title = "Míra zadlužení vůči počtu obytavel", caption = "Vodorovná osa představuje logaritmus počtu obyvatel,\n svislá dluh relativně vůči příjmům v roce 2019")
```
...
```{r rezervy-geo, fig.cap="Rezervy obcí v aglomeracích a ostatních územích", fig.asp= 0.4}
  ggplot(indik_geo) +
  geom_boxplot(aes(y = typobce_aglomerace, x = kratkodoby_fin_majetek/rozp_v_celkem, colour = typobce_aglomerace,
               fill = after_scale(alpha(colour, 0.7))), outlier.alpha = .2) +
  colorblindr::scale_colour_OkabeIto(guide = "none", order = c(7, 5, 2, 8),
                                    name = NULL, darken = 0) +
  scale_x_percent_cz(limits = c(0, 3)) +
...
```{r dluh-aglo, fig.cap="Deficit relativně k příjmům v aglomeracích a ostatních územích", fig.asp=.4}
  ggplot(indik_geo) +
  geom_boxplot(aes(y = typobce_aglomerace, x = dluh / rozp_p_celkem, colour = typobce_aglomerace, fill = after_scale(alpha(colour, 0.7))), outlier.alpha = .2) +
  colorblindr::scale_colour_OkabeIto(guide = "none", order = c(7, 5, 2, 8),
                                    name = NULL, darken = 0) +
  theme_studie("x") +
  scale_x_percent_cz() +
...
```{r deficit-podil-geo, fig.cap="Deficit relativně k příjmům v aglomeracích a ostatních územích", fig.asp=.4}
  ggplot(indik_geo) +
  geom_boxplot(aes(y = typobce_aglomerace, x = bilance/rozp_p_celkem, colour = typobce_aglomerace, fill = after_scale(alpha(colour, 0.7))), outlier.alpha = .2) +
  colorblindr::scale_colour_OkabeIto(guide = "none", order = c(7, 5, 2, 8),
                                    name = NULL, darken = 0) +
  theme_studie("x") +
  scale_x_percent_cz() +
...
```{r deficit-podil-aglo, fig.cap="Deficit relativně k příjmům v aglomeracích a metropolitních oblastech", fig.asp=.4}
  ggplot(indik_aglo) +
  geom_boxplot(aes(y = typ_aglomerace, x = bilance/rozp_p_celkem, colour = typ_aglomerace, fill = after_scale(alpha(colour, 0.7))), outlier.alpha = .2) +
  colorblindr::scale_colour_OkabeIto(guide = "none", order = c(7, 5, 2, 8),
                                    name = NULL, darken = 0) +
  theme_studie("x") +
  scale_x_percent_cz() +
